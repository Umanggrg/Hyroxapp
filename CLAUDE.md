...
- **Ingest wiring** — both `RaceViewModel.ingestHeartRate(_:)` and `FreeRunViewModel.ingestHeartRateBPM(_:at:)` already accept BPM+timestamp from arbitrary sources (Phase 27 made this universal). Just one more producer registered with the service. ~1 day. **Note**: `RaceViewModel`'s ingest is shared across the HYROX race flow AND Custom Workout / Simulation / Quick Station (all four route through the same engine + viewmodel with different `RaceKind` tags). Wiring the service into `RaceViewModel.ingestHeartRate` therefore unlocks Garmin HR for all four race-shaped workout types in one shot; `FreeRunViewModel.ingestHeartRateBPM` covers the fifth (Free Run). Five workout entry points off Train hub, two ingest funnels, one pairing flow.
